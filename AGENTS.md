...
Flama is a full-stack monorepo boilerplate built with Turborepo + pnpm. It contains 6 apps and 11 shared packages.
...
- `apps/web` imports only *types* from `@flama/shared`; its CJS build is not tree-shakeable by Rollup, so fetch runtime data (like the permission catalog) from the API
