# @flama/go-postgres — Agent Instructions
...
- A migration is a numbered `*.sql` file in the service's embedded migrations directory; it is applied forward-only, so a mistake is a new file.
- Repositories take the pool in their constructor; nothing else imports `pgx`.
...
```bash
pnpm --filter @flama/go-postgres lint    # golangci-lint run ./...
pnpm --filter @flama/go-postgres test    # go test -count=1 ./...
