# @flama/frontend-consumer — Agent Instructions
...
- Something the control plane also needs → promote it to
  `@flama/frontend-core` rather than copying it here.
...
pnpm --filter @flama/frontend-consumer build   # the apps import dist/
```

## Patterns agents get wrong

- Calling `useFlamaApp()` in a product hook. The kernel container getter only
  knows kernel services; a consumer hook reads `useConsumerApp().organizations`.
- Importing `@flama/frontend-admin` to reuse a role type or invalidate a list.
  `products-never-meet` fails; the meeting point is a kernel contract, the way
  member lists meet on `MEMBER_LISTS_KEY` from `@flama/frontend-core/react`.
