...
```ts
import { appInfo } from '@flama/nitro-app-info';
...
```bash
pnpm --filter @flama/nitro-app-info nitrogen   # regenerate bindings after editing the spec
pnpm --filter @flama/nitro-app-info lint
```

The native sources are `ios/` and `android/`; `FlamaAppInfo.podspec` and
