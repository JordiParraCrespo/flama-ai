...
- A new native method is added to the spec in `src/AppInfo.nitro.ts`, then to
  `ios/` (Swift) and `android/` (Kotlin), then `pnpm --filter @flama/nitro-app-info nitrogen`
...
- Anything that is not native belongs in `@flama/frontend-mobile`, not here.

## Before pushing

```bash
pnpm --filter @flama/nitro-app-info nitrogen
