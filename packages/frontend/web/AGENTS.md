# @flama/frontend-web — Agent Instructions
...
```bash
pnpm --filter @flama/frontend-web lint
pnpm --filter @flama/frontend-web test
pnpm --filter @flama/frontend-web arch
pnpm --filter @flama/frontend-web typecheck   # source-exported: no dist to build
```

## Patterns agents get wrong

- Importing `@flama/frontend-consumer` or `@flama/frontend-admin` to finish a
