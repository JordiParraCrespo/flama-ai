...
design system and `@flama/frontend-core`, never a product package: a
...
```bash
pnpm --filter @flama/frontend-web lint        # biome check src/
pnpm --filter @flama/frontend-web test        # vitest run
pnpm --filter @flama/frontend-web arch        # concern layering
pnpm --filter @flama/frontend-web typecheck   # tsc --noEmit; there is no build step
```
