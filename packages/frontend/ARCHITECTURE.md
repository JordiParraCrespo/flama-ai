...
```
                 @flama/shared          @flama/api-client
                       │                        │
                       └──────────┬─────────────┘
                                  ▼
                        @flama/frontend-core            design-system/<platform>
                                  │                              │
               ┌──────────────────┼──────────────────┐           │
               ▼                  │                  ▼           ▼
   @flama/frontend-consumer       │      @flama/frontend-admin   @flama/frontend-<platform>
               │                  │                  │           │
               └────────┐         │         ┌────────┘           │
                        ▼         ▼         ▼                    │
...
- A product package imports the kernel, `@flama/shared` and
  `@flama/api-client`. Never the other product, never a kit.
...
The app's `lib/flama.ts` is the composition root, and loading a product's
modules is what makes the app that product (`apps/web/src/lib/flama.ts`):
...
the way a kernel hook reads `useFlamaApp().auth`.
...
- **Product → kernel**: the second product needs the module or the query
  ...
  `TOKENS`, its getter to `FlamaApp`; a shared key alone goes to
...
3. `src/modules/things/things.repository.ts` — `@injectable()`, calls
   `@flama/api-client` services, maps DTOs to entities, each method under
