# @flama/backend-authz — Agent Instructions
...
```bash
pnpm --filter @flama/backend-authz test
pnpm --filter @flama/backend-authz build
pnpm --filter @flama/api arch      # the API's boundaries still hold
```
