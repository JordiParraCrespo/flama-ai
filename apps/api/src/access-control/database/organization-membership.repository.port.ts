 ...
 * acting *in*.
 *
 * Better Auth owns the `member` table; this port exists so any module granting
 * per-instance access can ask the one question it needs without depending on
 * the organizations façade (which delegates to `auth.api` and would drag a
 * session into a command handler).
 */
