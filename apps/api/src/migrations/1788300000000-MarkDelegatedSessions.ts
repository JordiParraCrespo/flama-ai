 ...
 * is a label the client chooses, so a browser that sent `flama-api-token/...`
...
export class MarkDelegatedSessions1788300000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // The label `DelegatedSessionAdapter` has always written: `flama-<kind>/<credential>`.
    await queryRunner.query(
      `UPDATE "session"
          SET "delegated" = true
        WHERE "delegated" = false
          AND ("userAgent" LIKE 'flama-api-token/%' OR "userAgent" LIKE 'flama-oauth/%')`,
    );
    await queryRunner.query(
