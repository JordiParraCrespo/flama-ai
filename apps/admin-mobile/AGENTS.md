...
```bash
pnpm --filter @flama/admin-mobile dev       # start Expo dev server
pnpm --filter @flama/admin-mobile ios
pnpm --filter @flama/admin-mobile android
pnpm --filter @flama/admin-mobile build:dev # EAS build (dev profile)
```
