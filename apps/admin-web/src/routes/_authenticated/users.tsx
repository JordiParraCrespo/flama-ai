...
type DialogState =
  ...
  | null;
type ConfirmState = Extract<
  NonNullable<DialogState>,
  { kind: 'ban' | 'unban' | 'sessions' | 'delete' }
>;
...
function UsersPage() {
  ...
  const assignedRoles = useMemo(
    () => new Map(rows.map((user, index) => [user.id, userRolesQueries[index]?.data ?? []])),
    [rows, userRolesQueries],
  );
  ...
  const confirmation = isConfirmation(dialog) ? dialog : null;
  ...
  async function confirmAction() {
    ...
    try {
      if (confirmation.kind === 'ban') await ban.mutateAsync({ id: confirmation.user.id });
      if (confirmation.kind === 'unban') await unban.mutateAsync(confirmation.user.id);
      if (confirmation.kind === 'sessions') await revoke.mutateAsync(confirmation.user.id);
      if (confirmation.kind === 'delete') await remove.mutateAsync(confirmation.user.id);
      setDialog(null);
    } catch {
      // ConfirmDialog renders the mutation error.
    }
  }
  ...
  return (
    <>
      ...
      {dialog?.kind === 'password' && (
        <SetPasswordDialog user={dialog.user} onClose={() => setDialog(null)} />
      )}
      {confirmation && (
        <ConfirmDialog
          title={t(`control.users.confirm.${confirmation.kind}.title`)}
          description={t(`control.users.confirm.${confirmation.kind}.description`, {
            name: confirmation.user.name,
          })}
          confirmLabel={t(`control.users.confirm.${confirmation.kind}.submit`)}
...
function isConfirmation(dialog: DialogState): dialog is ConfirmState {
  return (
    dialog?.kind === 'ban' ||
    dialog?.kind === 'unban' ||
