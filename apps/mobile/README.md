...
link scheme is `EXPO_PUBLIC_MOBILE_SCHEME` (default `flama`), which must match
`MOBILE_SCHEME` on the API.

```bash
pnpm docker:dev                          # Postgres + Redis
pnpm --filter @flama/api dev             # the API this app talks to
pnpm --filter @flama/mobile prebuild     # regenerate ios/ and android/
pnpm --filter @flama/mobile ios          # or: android
pnpm --filter @flama/mobile dev          # Expo dev client, --clear
pnpm --filter @flama/mobile lint         # Biome + tsc --noEmit
pnpm --filter @flama/mobile test         # Vitest
pnpm --filter @flama/mobile arch         # dependency-cruiser
pnpm --filter @flama/mobile build:dev    # EAS build (development profile)
```
