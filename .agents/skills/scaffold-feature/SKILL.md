...
description: Scaffold a feature in a Flama frontend app (apps/web, apps/admin-web, apps/mobile, apps/admin-mobile). Use when the user asks to add a screen, page, section, dialog, form or feature to a frontend app, or mentions a new UI area for a domain module. Generates the kind directories (screens, sections, dialogs, forms, components, hooks, lib, __tests__) and a first screen that pass `pnpm check:structure` and the app's dependency-cruiser rules.
---
...
2. `pnpm --filter @flama/<app> arch` and `pnpm check:structure` — the Stop
   hook runs both.
3. `pnpm --filter @flama/<app> lint` (Biome + typecheck) and a spec in
